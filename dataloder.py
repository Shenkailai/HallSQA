...
class SpeechQualityDataset(Dataset):
    ...
    def __init__(
        ...
        self.est_norm_mean = args['est_norm_mean']
        self.est_norm_std = args['est_norm_std']
    ...
    def _wav2fbank(self, filename):
        ...
        fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, sample_frequency=44100, use_energy=False,
    ...
    def __getitem__(self, index):
        ...
        if self.hallucinate:
            fbank_hall = torch.transpose(fbank_hall, 0, 1)
            # this is just to satisfy new torchaudio version, which only accept [1, freq, time]
            fbank_hall = fbank_hall.unsqueeze(0)
            # squeeze it back, it is just a trick to satisfy new torchaudio version
            fbank_hall = fbank_hall.squeeze(0)
            fbank_hall = torch.transpose(fbank_hall, 0, 1)

            # normalize the input for both training and test
            if not self.skip_norm:
                fbank_hall = (fbank_hall - self.est_norm_mean) / (self.est_norm_std * 2)
            # skip normalization the input if you are trying to get the normalization stats.
            else:
                pass
            
            return fbank, fbank_hall, y, index

        return fbank, y, index
